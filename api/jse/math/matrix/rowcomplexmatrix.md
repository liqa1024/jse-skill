...
> `jse.math.matrix.RowComplexMatrix`

**核心职能**：`final class extends BiDoubleArrayMatrix`。行主序复数矩阵实现，`ColumnComplexMatrix` 的对称实现。数据以 `double[2][]` 存储，行元素连续排列。`real()`/`imag()` 返回 `RowMatrix` 引用视图。转置返回 `ColumnComplexMatrix` 引用视图（零拷贝）。

```java
// === 静态工厂 ===
static RowComplexMatrix zeros(int aSize)
static RowComplexMatrix zeros(int aNumRows, int aNumCols)
static RowComplexMatrix ones(int aSize)
static RowComplexMatrix ones(int aNumRows, int aNumCols)

static Builder builder(int aNumCols)
static Builder builder(int aNumCols, int aInitSize)

// === 构造器 ===
RowComplexMatrix(int aNumRows, int aNumCols, int aShift, double[][] aData)
RowComplexMatrix(int aNumRows, int aNumCols, double[][] aData)
RowComplexMatrix(int aNumCols, double[][] aData)
...
final int nrows()
final int ncols()
protected RowComplexMatrix newZeros_(int aNumRows, int aNumCols)
RowComplexMatrix copy()
...
// === 行视图 ===
List<? extends ComplexVector> rows()
ComplexVector row(int aRow)
ComplexVector asVecRow()

// === 实部/虚部 ===
final RowMatrix real()
final RowMatrix imag()

// === 运算器 ===
final IComplexMatrixOperation operation()   // fill / assignRow / forEachRow / refTranspose -> ColumnComplexMatrix
...
final static class Builder {
    void addRow(IComplexVector aRow)
    void addRow(IVector aRow)
    void addRow(IComplexVectorGetter aRowGetter)
    void addRow(IVectorGetter aRowGetter)
    void trimToSize()
    RowComplexMatrix build()
}
```
